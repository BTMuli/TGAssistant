...
function convertImg(nameCard) {
  dataList.map(async (item) => {
    ...
    if (!fs.existsSync(`${item.srcDir}/${nameCard.index}.webp`)) {
      defaultLogger.error(
        `[名片][转换][${transI(nameCard.index)}] 名片 ${nameCard.name} ${item.type} 图像不存在`,
      );
      return;
    }
    // item.srcDir/index.webp -> item.outDir/name.webp
    if (item.type === "icon") {
      await sharp(`${item.srcDir}/${nameCard.index}.webp`)
        .webp()
        .resize(250, 165)
        .toFile(`${item.outDir}/${nameCard.name}.webp`);
    ...
    consoleLogger.mark(
      `[名片][转换][${transI(nameCard.index)}] 名片 ${nameCard.name} ${item.type} 图像转换成功`,
    );
  });
}
...
function getNameCardType(nameCard) {
  ...
  try {
    ...
    defaultLogger.error(
      `[名片][下载][${transI(nameCard.index)}] 名片 ${nameCard.name} source 数据类型错误`,
    );
  }
