...
defaultLogger.info(
  `[材料][转换] amber.json 处理完成，共 ${count.total} 条数据，成功 ${count.success} 条，失败 ${count.fail} 条，跳过 ${count.skip} 条`,
);
...
await Promise.all(
  amberKey.map(async (key) => {
    ...
    if (fileExist(out)) {
      count.skip++;
      consoleLogger.mark(
        `[材料][转换][${key}] ${amberJson["items"][key]["name"]} 目标图片已存在，跳过`,
      );
      return;
    }
...
defaultLogger.info(
  `[材料][转换] icon 处理完成，共 ${count.total} 条数据，成功 ${count.success} 条，失败 ${count.fail} 条，跳过 ${count.skip} 条`,
);
