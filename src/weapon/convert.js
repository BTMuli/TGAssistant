...
defaultLogger.info(`[武器][转换] amber.json 处理完成，共 ${count.total} 个，成功 ${count.success} 个，失败 ${count.fail} 个，跳过 ${count.skip} 个`);
count.success = 0;
count.fail = 0;
count.skip = 0;
// 处理 mys.json
...
await Promise.allSettled(weaponData.map(async (weapon) => {
